class Permissions {
    ...
    SettingPermissionsOff()
    {
       ...
       Logout()
       {
        ...
        cy.contains('Logout').realHover().wait(1000).click().realMouseMove(500,500)
       }
       GameUserLogin()
       {
        cy.get('#email').type('f.faisalathar+gameuser@gmail.com').wait(1000)
        cy.get('#password').type('Asdf@1234').wait(1000)
        cy.get('.button').realHover().click().wait(3000)
       }
       CheckingPermissionsOff()
       {
        ...
        cy.contains('Content').scrollIntoView()
     ...
     SettingPermissionsOn()
    {
       ...
       CheckingPermissionsOn()
       {
        cy.get('.projects-name').realHover({scrollBehavior:false}).wait(1000)
        cy.contains('Overview').scrollIntoView().wait(2000)
        cy.contains('Link Builder').should('exist')
        cy.contains('Post Campaign').should('exist').wait(1000)
