...
class funnelTypes{
   StripeMethod()
    ...
    cy.iframe("[title='Secure payment input frame']").find('#Field-linkMobilePhoneInput').type('(236) 555-5523').wait(2000)
    //cy.iframe("[title='Secure payment input frame']").find('#Field-linkLegalNameInput').type('Faisal Athar').wait(2000)  
    cy.get('#card-button').scrollIntoView().wait(1000).click({force:true}).invoke('text','Processing...')
