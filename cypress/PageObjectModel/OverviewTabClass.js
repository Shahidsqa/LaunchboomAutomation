    ...
    RedirectionToggle()
    {
     ...
     FontsSelection()
      ...
      SaveSettings()
      {
        ...
        cy.wrap($paragraph).should('have.css', 'font-family', '"Playwrite IS"')}).wait(1000)
        cy.go(-1).wait(2000)
        cy.get('.btn').scrollIntoView().wait(1000).click().wait(4000)
        // Verifying fonts on Checkout page
        cy.get('h1, h2, h3, h4, h5, h6').each(($heading) => {
        cy.wrap($heading).should('have.css', 'font-family', 'Montserrat, sans-serif')}).wait(1000)
        cy.get('p').not('.foot-wrap p').each(($paragraph) => {
        cy.wrap($paragraph).should('have.css', 'font-family', 'Montserrat, sans-serif')}).wait(1000)
        cy.get('input').each(($input) => {
        cy.wrap($input).should('have.css', 'font-family', 'Montserrat, sans-serif')}).wait(1000)
        cy.get('button').each(($button) => {
        cy.wrap($button).should('have.css', 'font-family', 'Montserrat, sans-serif')}).wait(1000)
        cy.get('label').each(($label) => {
        cy.wrap($label).should('have.css', 'font-family', 'Montserrat, sans-serif')}).wait(1000)
       ...
       // cy.iframe("[title='Secure payment input frame']").find("[aria-label='Close']").click().wait(2000)
        ...
        cy.get('#card-button').scrollIntoView().wait(2000).click({scrollBehavior:false}).wait(5000)
     ...
     FollowerFlowFontsChecking()
     {
        ...
        cy.get('.IDL5I64BAI9W9TJ0 > #top-cta-location > .v4-n--inner > .v4-n--inn-align > .v4-n--bottom-content-wrap > .v4-n--cta-inputs-wrap > .sign-up-form > .email').type('f.faisalathar@gmail.com',{scrollBehavior:false})
        cy.wait(2000)
        cy.get('.IDL5I64BAI9W9TJ0 > #top-cta-location > .v4-n--inner > .v4-n--inn-align > .v4-n--bottom-content-wrap > .v4-n--cta-inputs-wrap > .sign-up-form > .cta-form-submit').click({scrollBehavior:false}).wait(3000)
